const runners = [
  ...
  // const shirtOrder = runners.map(function(item){
  ...
  // const shirtOrder = runners.map(item => `${item.last_name}, ${item.shirt_size}`);
  ...
  for(let i = 0; i < runners.length; i++){
    shirtOrder.push(`${runners[i].last_name}, ${runners[i].shirt_size}`);
  }
